# control_final_demo.py (Versión Final Completa con Misiones y Bucle Robusto)
import os
import sys
import time
import json
...
# ------------------------------------------------------------------
# Configuración de la lógica inteligente
...
# ¡¡¡IMPORTANTE!!! CAMBIA ESTOS VALORES POR IDs DE CALLES (EDGES) VÁLIDOS Y DISTANTES DE TU MAPA
CALLE_INICIO_EMERGENCIA = "-E21"  # Ejemplo: Esquina superior izquierda
CALLE_FIN_EMERGENCIA = "E24"   # Ejemplo: Esquina inferior derecha

# ------------------------------------------------------------------
# --- SCRIPT PRINCIPAL ---
# ------------------------------------------------------------------
SUMO_CONFIG_FILENAME = "calles.sumocfg"
# Usamos sumo-gui para la demo visual
sumo_cmd = ["sumo-gui", "-c", SUMO_CONFIG_FILENAME, "--start", "--quit-on-end"]

print("Iniciando DEMO FINAL de SYNCRONÍA (con Misiones)...")
...
try:
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        # --- Lógica de activación de emergencia desde el Dashboard (MISIONES) ---
        if not VEHICULO_EMERGENCIA_ID and os.path.exists("emergency_request.txt"):
            ...
            try:
                ...
                traci.vehicle.setSpeedMode(id_ambulancia, 31)
                traci.vehicle.setLaneChangeMode(id_ambulancia, 512)
                
                VEHICULO_EMERGENCIA_ID = id_ambulancia
                print(f"\n¡¡¡ MISIÓN DE EMERGENCIA CREADA !!! Despachando ambulancia '{VEHICULO_EMERGENCIA_ID}'.")
                
            except traci.TraCIException as e:
                print(f"Error al crear la misión de emergencia: {e}. ¿Son correctas las calles de inicio y fin?")
                VEHICULO_EMERGENCIA_ID = None
